...
int Yolov5_Model::serialize_engine(std::string wts_name, std::string engine_name, bool is_p6, float gd, float gw)
{
...
int Yolov5_Model::deserialize_engine(std::string engine_name)
{
...
int Yolov5_Model::get_width(int x, float gw, int divisor = 8) {
    return int(ceil((x * gw) / divisor)) * divisor;
}
int Yolov5_Model::get_depth(int x, float gd) {
    if (x == 1) return 1;
    int r = round(x * gd);
...
ICudaEngine* Yolov5_Model::build_engine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt, float& gd, float& gw, std::string& wts_name) {
    ...
    config->setMaxWorkspaceSize(1024 * (1 << 20));  // 16MB
    if (int8_flag)
    {
...
ICudaEngine* Yolov5_Model::build_engine_p6(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt, float& gd, float& gw, std::string& wts_name) {
    INetworkDefinition* network = builder->createNetworkV2(0U);
    // Create input tensor of shape {3, INPUT_H, INPUT_W} with name INPUT_BLOB_NAME
    ITensor* data = network->addInput(INPUT_BLOB_NAME, dt, Dims3{ 3, INPUT_H, INPUT_W });
    assert(data);
...
void Yolov5_Model::APIToModel(unsigned int maxBatchSize, IHostMemory** modelStream, bool& is_p6, float& gd, float& gw, std::string& wts_name) {
    // Create builder
...
void Yolov5_Model::doInference(IExecutionContext& context, cudaStream_t& stream, void **buffers, float* output, int batchSize) {
...
std::vector<Yolo::Detection> Yolov5_Model::run_frame(cv::Mat img)
{
...
int Yolov5_Model::run_frames(std::vector<cv::Mat> imgs_buffer)
{

}

int Yolov5_Model::run_files(std::string img_dir = "../testimgs")
{
...
int Yolov5_Model::build(std::string wts_name, std::string engine_name, bool is_p6, float gd, float gw, std::string img_dir)
{
    serialize_engine(wts_name, engine_name, is_p6, gd, gw);
}
